...
**MSP probe (interim).** `/v2/me` does not expose `is_msp_account` yet, so the probe calls `GET /v2/managed_accounts?page=1&limit=1`. Success → `{ msp: true }`; the known denial message *"You do not have permission to access this resource."* → `{ msp: false }`; any other failure throws `MCPUserError` (do not treat transient errors as non-MSP). Long-term fix: expose `is_msp_account` on `/v2/me` and switch the probe.
