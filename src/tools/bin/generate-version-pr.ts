...
const versionTagExists = runCommand("git", [
    "tag", "-l", "--points-at", "HEAD", `v${serverMeta.version}`
]).trim().length > 0;
if (!versionTagExists) {
    console.log(`Version tag v${serverMeta.version} does not exist - not going to try and version bump and deleting automated-bump branch if it exists`);
...
function checkIfChanges(tagTool: Tool, mainTool: Tool): boolean {
    ...
    if (
        JSON.stringify(tagTool.annotations) !== JSON.stringify(mainTool.annotations)
    ) {
        return true;
    }
    if (
...
async function doPr(description: string, newVersion: string) {
    ...
    runCommand("gh", ["pr", "create", "--repo", "vantage-sh/vantage-mcp-server", "--title", `chore: Bump version to ${newVersion}.`, "--body", description, "--base", "main", "--head", "automated-bump"]);
}
