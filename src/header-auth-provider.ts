...
export class HeaderAuthProvider {
  ...
  private matchesApiRoute(url: URL): boolean {
    const routes = Array.isArray(this.options.apiRoute) ? this.options.apiRoute : [this.options.apiRoute];

    for (const route of routes) {
      if (route.startsWith("http")) {
        const routeUrl = new URL(route);
        if (url.hostname === routeUrl.hostname && url.pathname.startsWith(routeUrl.pathname)) {
          return true;
        }
      } else {
