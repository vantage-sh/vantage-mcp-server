  ...
  const toolHandlerContext: TestHandlerContext<Input, Output> = {
    ...
    callExpectingMCPUserError: async (args) => {
      try {
        ...
        if (e instanceof Error) {
          const error = new Error(`Expected MCPUserError, but got generic error: ${e.message}`);
...
export function requestsInOrder(reqs: AnyExpectedApiCall[]): ToolCallContext["callVantageApi"] {
  ...
  >(
    ...
    if (callIndex >= reqs.length) {
      throw new Error(`Unexpected API call to ${endpoint} with params ${JSON.stringify(params)}`);
    }
    ...
    expect(params, "comparing given API Params with actual params in API call").toEqual(expected.params);
    callIndex++;
...
export function poisonOneValue<
  ...
  DataType,
  Object extends InferValidators<SchemaTestTableItemValidators> = InferValidators<SchemaTestTableItemValidators>,
>(
  obj: Object,
  key: {
    [K in keyof Object]: DataType extends Object[K] ? K : Object[K] extends DataType ? K : never;
  }[keyof Object],
  poisoner: {
    value: DataType;
